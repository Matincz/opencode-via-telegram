...
export type TelegramMediaCoreOptions = {
  defaultCacheRoot?: string | (() => string)
...
type DownloadAttachmentInput = {
  ...
  targetDir: string
  options?: TelegramMediaCoreOptions
}

const DEFAULT_MIME_BY_EXTENSION: Record<string, string> = {
  ".jpg": "image/jpeg",
  ".jpeg": "image/jpeg",
  ".png": "image/png",
  ".webp": "image/webp",
...
function getMimeByExtension(options?: TelegramMediaCoreOptions) {
  return {
    ...DEFAULT_MIME_BY_EXTENSION,
    ...options?.mimeByExtension,
  }
}

function getMaxDownloadBytes(options?: TelegramMediaCoreOptions) {
  return options?.maxDownloadBytes ?? TELEGRAM_DOWNLOAD_LIMIT_BYTES
}
...
function inferExtensionFromMime(mime: string, mimeByExtension: Record<string, string>) {
  const lower = mime.toLowerCase()
  const match = Object.entries(mimeByExtension).find(([, value]) => value === lower)
  return match?.[0] ?? ""
}

function inferMimeFromFilename(filename: string, mimeByExtension: Record<string, string>) {
...
function defaultFilename(
  ...
  const ext = inferExtensionFromMime(mime ?? attachment.mime ?? defaultMimeForKind(attachment.kind), mimeByExtension)
  return sanitizeFilename(`${attachment.kind}-${attachment.fileUniqueId ?? attachment.messageId}${ext}`)
}

export async function resolveUniqueTargetPath(targetDir: string, filename: string) {
...
async function downloadAttachment(input: DownloadAttachmentInput): Promise<ResolvedTelegramAttachment> {
  ...
  const maxDownloadBytes = getMaxDownloadBytes(input.options)
  if ((input.attachment.fileSize ?? 0) > maxDownloadBytes) {
  ...
  const mimeByExtension = getMimeByExtension(input.options)
  const contentType = response.headers.get("content-type")?.split(";")[0]?.trim()
  const mime =
    input.attachment.mime ||
    contentType ||
    inferMimeFromFilename(file.file_path, mimeByExtension) ||
    defaultMimeForKind(input.attachment.kind)
  const filename = defaultFilename(input.attachment, file.file_path, mime, mimeByExtension)
...
function resolveDefaultCacheRoot(defaultCacheRoot?: string | (() => string)) {
  if (typeof defaultCacheRoot === "function") return defaultCacheRoot()
  if (typeof defaultCacheRoot === "string" && defaultCacheRoot.trim()) return defaultCacheRoot
  return path.join(process.cwd(), ".cache", "telegram-media")
}

export function getMediaCacheRoot(
  overrideRoot?: string,
  options?: Pick<TelegramMediaCoreOptions, "defaultCacheRoot">,
) {
  return overrideRoot?.trim() || resolveDefaultCacheRoot(options?.defaultCacheRoot)
}
