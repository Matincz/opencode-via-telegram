...
export function loadSelectedModels() {
  try {
    if (!fs.existsSync(SELECTED_MODELS_FILE)) return
...
export function loadActiveProjects() {
  try {
    ...
    const parsed = JSON.parse(fs.readFileSync(ACTIVE_PROJECTS_FILE, "utf-8"))
    for (const key of Object.keys(parsed)) {
...
export function saveActiveProjects() {
  try {
    ...
    fs.writeFileSync(ACTIVE_PROJECTS_FILE, JSON.stringify(obj, null, 2), "utf-8")
  } catch (err) {
