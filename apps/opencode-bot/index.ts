...
import { installGlobalLogger, logInfo, logWarn } from "./src/runtime/logger"
...
const TELEGRAM_POLLING_WATCHDOG_INTERVAL_MS = Number(process.env.TELEGRAM_POLLING_WATCHDOG_INTERVAL_MS || "15000")
const TELEGRAM_PENDING_UPDATE_STALL_MS = Number(process.env.TELEGRAM_PENDING_UPDATE_STALL_MS || "30000")
...
let lastTelegramInboundAt = Date.now()

function markTelegramInbound() {
...
const pollingWatchdog = createTelegramPollingWatchdog({
  bot,
  intervalMs: TELEGRAM_POLLING_WATCHDOG_INTERVAL_MS,
  stalledPendingThresholdMs: TELEGRAM_PENDING_UPDATE_STALL_MS,
  getLastInboundAt: () => lastTelegramInboundAt,
})

pollingWatchdog.start()
stopPollingWatchdog = () => pollingWatchdog.stop()
void pollingWatchdog.checkNow()
...
bot.on("callback_query", async (query: any) => {
  ...
  await handleTelegramCallbackQuery(query, {
    ...
    hasActiveTrackedResponse: streaming.hasActiveTrackedResponse,
