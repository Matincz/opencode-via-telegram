...
export function createProjectSessionManager(context: ProjectSessionManagerContext) {
  ...
  async function getActiveProjectWorktree(chatId: number): Promise<string | undefined> {
    ...
    if (explicit && explicit !== "__current__") {
      if (path.isAbsolute(explicit)) {
        const worktree = path.resolve(explicit)
      ...
      const projects = await context.listProjects().catch(() => [])
    ...
    if (typeof current?.worktree === "string") {
  ...
  async function ensureSession(chatId: number): Promise<string> {
    ...
    const task = (async () => {
      ...
      let worktree = await getActiveProjectWorktree(chatId)
      const backend = await context.resolveOpencodeBackend().catch(() => null)

      if (!worktree && backend && backend.source !== "env") {
        const fallbackCurrent = await context.opencodeGet("/project/current").catch(() => null)
        if (typeof fallbackCurrent?.worktree === "string") {
          worktree = fallbackCurrent.worktree
        } else {
  ...
  async function revertLastUserMessage(chatId: number) {
    const sessionId = sessionMap.get(chatId)
    if (!sessionId) throw new Error("当前没有活跃会话。")

    const messages = await context.opencodeGet(`/session/${sessionId}/message`, chatId, true).catch(() => [])
    const lastUserMessage = Array.isArray(messages)
      ? [...messages].reverse().find((message: any) => message?.info?.role === "user" && typeof message?.info?.id === "string")
      : null

    if (!lastUserMessage?.info?.id) {
      throw new Error("当前会话没有可撤销的消息。")
    }
  ...
  async function unrevertSession(chatId: number) {
    const sessionId = sessionMap.get(chatId)
    if (!sessionId) throw new Error("当前没有活跃会话。")
