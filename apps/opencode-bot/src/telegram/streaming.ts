...
export function createTelegramStreaming(context: TelegramStreamingContext) {
  ...
  const RESPONSE_TIMEOUT_NOTICE = "⚠️ 当前响应等待超时，请稍后重试，或发送 /stop 后重新开始。"
  ...
  function hasActiveTrackedResponse(chatId: number, sessionId: string) {
    const state = chatStates.get(chatId)
    return !!state && state.responseMode !== null && state.responseSessionId === sessionId
  }
  ...
  function finalizeTrackedResponse(
    chatId: number,
    sessionId: string,
    nonce: number,
    options?: { clearDrafts?: boolean },
  ) {
    if (!isTrackedResponseActive(chatId, sessionId, nonce)) return false
    pendingUserTexts.delete(sessionId)
    stopTypingIndicator(chatId)
    clearResponseTracking(chatId)
    if (options?.clearDrafts !== false) clearDrafts(chatId)
    return true
  }

  async function handleResponseError(chatId: number, sessionId: string, nonce: number, error: unknown) {
    if (!finalizeTrackedResponse(chatId, sessionId, nonce)) return
  ...
  async function pollSessionResponse(
    ...
    while (Date.now() - startedAt < context.opencodeResponsePollTimeoutMs) {
      ...
      for (const message of newAssistantMessages) {
        ...
        logInfo("TG.POLL.REPLY", {
          chatId,
          sessionId,
          assistantMessageId: message.info.id,
          hasText: synced.hasText,
        })
        ...
        finalizeTrackedResponse(chatId, sessionId, nonce)
        return
      }
    ...
    await context.bot.sendMessage(chatId, RESPONSE_TIMEOUT_NOTICE).catch(() => { })
  }
  ...
  function createScopedSessionResponseStream(
    ...
    let finished = false
    const finish = (options?: { clearDrafts?: boolean }) => {
      if (finished) return false
      const didFinalize = finalizeTrackedResponse(chatId, sessionId, nonce, options)
      if (didFinalize) finished = true
      return didFinalize
    }
    ...
    const startWatchdog = () => {
      ...
      timeoutTimer = setTimeout(async () => {
        if (!finish()) return
        await context.bot.sendMessage(chatId, RESPONSE_TIMEOUT_NOTICE).catch(() => { })
      }, context.opencodeResponsePollTimeoutMs)
    }
    ...
    const task = (async () => {
      ...
      while (isTrackedResponseActive(chatId, sessionId, nonce)) {
        try {
          ...
          startWatchdog()
          logInfo("TG.SSE.SCOPED_CONNECTED", { chatId, sessionId, worktree })
            ...
            for (const chunk of chunks) {
              ...
              try {
                ...
                if (payload.type === "session.error") {
                  ...
                  logError("TG.SESSION.ERROR", { chatId, sessionId }, errorMessage || "unknown")
                  if (!finish()) return
          ...
          logWarn("TG.SSE.SCOPED_ERROR", { chatId, sessionId }, error)
      ...
      while (state.bubbleOrder.length > 0) {
        ...
        if (text) {
          ...
          try {
            ...
            console.error(
              `[BUBBLE_SEND_ERROR] chat=${chatId} bubble=${bubbleId} partType=${bubble.partType} reason=${error?.message || error}`,
            )
          }
  ...
  async function listenEvents() {
    ...
    while (true) {
      try {
        ...
        logInfo("TG.SSE.GLOBAL_CONNECTED")
        ...
        while (true) {
          ...
          for (const chunk of chunks) {
            ...
            try {
              ...
              if (payload.type === "session.error") {
                const errorMessage = extractSessionErrorMessage(payload)
                logError("TG.SESSION.ERROR", { chatId: targetChatId, sessionId: sessionID }, errorMessage || "unknown")
                pendingUserTexts.delete(sessionID)
