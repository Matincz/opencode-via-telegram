...
export interface TelegramCallbackQueryContext {
  ...
  hasActiveTrackedResponse: (chatId: number, sessionId: string) => boolean
...
export async function handleTelegramCallbackQuery(
  ...
  if (!chatId || !messageId) {
    await answerCallbackQuerySafe(context.bot, query.id)
    return
  }
  ...
  if (data.startsWith("prm:")) {
    ...
    if (!reqInfo) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 审批请求已过期")
      return
    }
    ...
    try {
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 操作失败")
      return
    }
  ...
  if (data.startsWith("q:")) {
    ...
    if (!action) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 问题已过期，请重新触发")
      return
    }
    ...
    try {
      if (action.type === "reply") {
        ...
        if (state && context.hasActiveTrackedResponse(chatId, state.sessionId)) {
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 问题处理失败")
      return
    }
  ...
  if (data.startsWith("session:")) {
    ...
    if (!newSessionId) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 会话信息已过期，请重新获取")
      return
    }
  ...
  if (data.startsWith("project:")) {
    ...
    if (!payload || payload.type !== "project") {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 项目信息已过期，请重新获取")
      return
    }
    ...
    if (
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 该项目范围过大，请选择更具体的仓库目录")
      return
    }
  ...
  if (data === "p:back") {
    try {
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 获取列表失败")
      return
    }
  ...
  if (data.startsWith("provider:")) {
    ...
    if (!providerId) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 供应商信息已过期，请重新获取")
      return
    }
    ...
    try {
      ...
      if (!provider || !provider.models) {
        await answerCallbackQuerySafe(context.bot, query.id, "❌ 该供应商下无模型")
        return
      }
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 获取模型失败")
      return
    }
  ...
  if (data.startsWith("model:")) {
    ...
    if (!modelId) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 模型信息已过期，请重新获取")
      return
    }
    ...
    try {
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 切换模型失败")
      return
    }
  ...
  if (data.startsWith("command:")) {
    ...
    if (!cmdName || !sessionId) {
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 当前无会话")
      return
    }
    ...
    try {
      ...
      await answerCallbackQuerySafe(context.bot, query.id, "❌ 命令执行失败")
      return
    }
