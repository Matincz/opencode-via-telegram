...
function createArtifactFile(prefix: string, content: string) {
  ensureOutputDir()
  const safePrefix = prefix.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "codex-output"
...
export async function deliverCodexTextResult(input: {
  bot: TelegramBot
  chatId: number
  text: string
  prefix: string
  ...
  const text = input.text.trim() || "Codex 已完成，但没有返回可显示的正文。"
