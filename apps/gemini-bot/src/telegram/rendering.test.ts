...
describe("createDraftSender", () => {
  it("turns empty draft updates into a zero-width-space clear signal", async () => {
...
describe("sendRenderedAssistantPart", () => {
  ...
  it("renders status updates as execution status and deletes them after sending", async () => {
    globalThis.setTimeout = ((fn: (...args: any[]) => void) => {
      fn()
      return 0 as any
    }) as typeof setTimeout
    ...
    expect(renderedMessage).toContain("执行状态")
    expect(deleteMessage).toHaveBeenCalledWith(1, 7)
  })
})
